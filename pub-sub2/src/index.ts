...
function auth(req:any,res:any,next:any){
    ...
    const user = jwt.verify(token,"secretkey") as {id:number,username:string};
    req.user = user;
    next();
    }
    catch(error){
...
app.post("/placeorder",auth,async(req:any,res:any)=>{
    ...
    if(balance<margin) return res.status(401).json({message:"insufficient fund"});
    //replace this
...
app.get("/balance",auth,async(req:any,res:any)=>{
    const user = req.user;
    try{
...
app.get("/order-history",auth,async(req:any,res:any)=>{
    const user = req.user;
    try{
...
app.post("/close-order",auth,async (req:any,res:any)=>{
...
redis.subscribe("trades",(err,count)=>{
    if (err) console.error("there was issue with subcribing to redis")
    else console.log(`subscribed to ${count} channel`);
});
