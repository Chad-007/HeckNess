...
app.post("/signup", async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query("INSERT INTO users (username, password_hash, balance) VALUES ($1, $2, $3) RETURNING id, username, balance", [username, hashed, 10000]);
...
app.post("/placeorder", authMiddleware, async (req, res) => {
    ...
    try {
       ...
       RETURNING *`, [user.id, symbol, type, entryPrice, quantity, orderAmount, expiry, tp, sl]);
...
app.get("/orders", authMiddleware, async (req, res) => {
    const user = req.user;
    try {
        const ordersRes = await pool.query("SELECT * FROM orders WHERE user_id=$1 AND status='active' ORDER BY created_at DESC", [user.id]);
...
app.post("/close-order", authMiddleware, async (req, res) => {
    const { orderId } = req.body;
    const user = req.user;
    try {
        // Get the order
        const orderRes = await pool.query("SELECT * FROM orders WHERE id=$1 AND user_id=$2 AND status='active'", [orderId, user.id]);
...
redisSubscriber.on("message", async (_channel, message) => {
    ...
    try {
        ...
        const expiredOrdersRes = await pool.query("SELECT * FROM orders WHERE expiry <= NOW() AND status='active'");
